...
The first preview exposed extensionless ESM imports that Vite allowed but Vercel's Node runtime rejected. Relative imports in the API dependency graph now use explicit `.js` extensions. The new `tsconfig.api.json` checks NodeNext module resolution so this mismatch is caught before deployment.

The corrected preview at commit `1fcaf44` returned `configured: true` from `/api/chat`. A real hosted introduction produced 227 streamed text deltas and a model-complete event with valid sources, both employers, measurable results, and recognition. The production domain is checked again after the GitHub-linked main deployment; its outcome is recorded in the release task.
